...
public class AppChooser {
	...
	/**
	 * Creates a chooser that only shows installed apps that are allowed by the
	 * whitelist.
	 * 
	 * @param pm
	 ...
	 * @param whitelist
	 *            A list of package names that are allowed to show.
	 * @return Updated intent, to be passed to
	...
	public static Intent create(PackageManager pm, Intent target, String title,
		...
		for (ResolveInfo ri : resInfo) {
			if (ri.activityInfo == null
					|| !whitelist.contains(ri.activityInfo.packageName))
				continue;
			...
			info.put("simpleName",
					String.valueOf(ri.activityInfo.loadLabel(pm)));
			metaInfo.add(info);
		}
		...
		if (metaInfo.isEmpty()) {
			...
			emptyIntent.setPackage("com.product.islah");
			emptyIntent
					.setClassName("com.product.islah", "NonExistingActivity");
		...
		for (HashMap<String, String> mi : metaInfo) {
			...
			targetedShareIntent.setClassName(mi.get("packageName"),
					mi.get("className"));
			targetedIntents.add(targetedShareIntent);
		}

		Intent chooserIntent = Intent.createChooser(targetedIntents.get(0),
				title);
